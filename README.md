...
```text
   ...
   [ Read EEPROM Flash Memory ]
    ...
    [ Display "Enter Password" ]
               │
               ▼
    [ Awaiting Tactile Input ] ◄──────────────────────────────┐
               │                                              │
      ┌────────┴────────┐                                     │
      ▼                 ▼                                     │
  [ Key Pressed ]   [ '#' Modifier Key ]                      │
      │                 │                                     │
      ▼                 ▼                                     │
[ Buffer Digit ]  [ Current Pass Challenge ]                  │
      │                 │                                     │
      │        ┌────────┴────────┐                            │
      │        ▼                 ▼                            │
      │   [ Verified ]     [ Unverified ]                     │
      │        │                 │                            │
      │        ▼                 ▼                            │
      │   [ Write New ]    [ Flash Red LED ] ─────────────────┤
      │   [ Password  ]    [ Buzz Warning  ]                  │
      │   [ To EEPROM ]          │                            │
      │        │                 ▼                            │
      │        └───────────► [ Reset UI ]                     │
      │                                                       │
...
[ Check Credentials ]
  ...
  ( Match )        ( Mismatch )
      │                 │
      ▼                 ▼
[ Access Granted ]   [ Increment Strike Counter ]
[ Green LED HIGH ]      │
[ Servo Unlatch  ]      ▼
[ Wait 5 Seconds ]   [ Strikes >= 3? ]
[ Servo Relatch  ]      │
      │                 ├─────────────────┐
      │                 ▼                 ▼
      │              ( Yes )            ( No ) ────────────────┐
...
#### 16x2 Alphanumeric Display (Parallel Bus Config)
* **RS (Register Select)** ➔ Analog Input `A0`
* **E (Enable)** ➔ Analog Input `A1`
* **D4 (Data Bit 4)** ➔ Analog Input `A2`
* **D5 (Data Bit 5)** ➔ Analog Input `A3`
* **D6 (Data Bit 6)** ➔ Analog Input `A4`
* **D7 (Data Bit 7)** ➔ Analog Input `A5`
* **VSS / R_W / LED_K** ➔ Common Circuit Ground `GND`
* **VDD / LED_A** ➔ Constant Power Rail `5V` (LED_A inline with 220Ω resistor)
* **V0 (Contrast Pin)** ➔ Common Circuit Ground `GND` (Direct binding for high-definition rendering)

#### 4x4 Matrix Input Tactile Keyboard Array
* **Row 1** ➔ Digital I/O `Pin 8` *(Isolated from UART line interference)*
* **Row 2** ➔ Digital I/O `Pin 1`
* **Row 3** ➔ Digital I/O `Pin 2`
* **Row 4** ➔ Digital I/O `Pin 3`
* **Column 1** ➔ Digital I/O `Pin 4`
* **Column 2** ➔ Digital I/O `Pin 5`
* **Column 3** ➔ Digital I/O `Pin 6`
* **Column 4** ➔ Digital I/O `Pin 7`

#### Peripherals & Output Actuators
* **Servo Latch Signal Trace** ➔ Digital Output `Pin 9` (PWM Node)
* **Green Access LED** ➔ Digital Output `Pin 10`
* **Red Access LED** ➔ Digital Output `Pin 11`
* **Acoustic Piezo Buzzer** ➔ Digital Output `Pin 12`
...
void loop() {
  ...
  if (data_count == 4) {
    ...
    if (strcmp(Data, Master) == 0) {
      ...
      myservo.write(90); // Displacement vector matched to clear barrier alignment
      beep(300);
      delay(5000); 
      myservo.write(0); // Actuate barrier configuration to static lock alignment
...
| Tactile Scanning | 4x4 matrix indexing latency tracking ($<5\,\text{ms}$) | **Passed** |
| Visual UI Echo | Character array masking (`*` character parsing) | **Passed** |
| Access Authorization | Validation comparison matching execution ($0\,\text{ms}$ delay) | **Passed** |
| Barrier Actuation | PWM mechanical displacement angle matching (0° to 90°) | **Passed** |
| Lockout Quarantine | Hard execution halt tracking ($30000\,\text{ms}$ loop cutoff) | **Passed** |

---

## 🚀 Applications

The standalone architectural blueprint of this digital lock controller makes it suitable for integration within real-world applications including:

- Residential Secure Entrance Portals
- Laboratory Intrusion Prevention Systems
- Academic Embedded Design Curriculums
...
Through the assembly and software engineering phases of this microcontroller project, the following core concepts were successfully mastered:

- Array manipulation and variable-state buffer tracking in C++.
- Matrix keyboard structural column/row electrical multiplexing.
- String optimization logic via memory functions (`strcmp`, `memset`).
- Interactivity management handling hardware serial line isolation.
- Microcontroller persistent byte sector allocation operations via EEPROM manipulation.
...
This repository is published exclusively for **educational and academic purposes**. It may be utilized for individual learning, system references, and academic research with appropriate attribution.

---

<div align="center">

### ⭐ If you found this project layout helpful, consider leaving a star!
